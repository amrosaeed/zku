...
// A class implementation of a database for sotring witnesses for block with indexes, with public visibility functions 
// "fn new" & "fn start" to reserve a db slot, and "fn next_witness_block" for returning next block for generating witness
// "async fn should_work_on_block" async function runs in a single thread Returns status of witness for block with 
// index block_number
// "async fn load_account_tree" async function that loads merkle tree leaf for single account
// "async fn prepare_witness_and_save_it" async function that mutates on certain conditions and pins "circuit_account_tree"
// and database slots.
