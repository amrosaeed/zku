...
// Struct "ETHSenderConfig" have an extra level of visibility with its fields "Sender, gas_price_limit" 
// it's visibility  overridden with the pub modifier. the "ETHSenderConfig" struct is accessed from outside, 
// and it's main goal to configure the Ethereum sender crate
...
// A class implementation of "ETHSenderConfig" has a method "pub fn from_env" with the keyword pup that makes 
// the function accessible from inside external modules, and got the keyword "self" that constructs a profile 
// from the value of the environment variables ["eth_sender", "ETH_SENDER_SENDER_"] for the channel "sender:" 
// & environment variables ["eth_sender.gas_price_limit","ETH_SENDER_GAS_PRICE_LIMIT_"] for the channel 
// "gas_price_limit:" . the  that takes as input miniblock_interval.
// In short it waits for both "impl Sender" and "impl GasLimit" to monitor the block finalization to the 
// corresponding Ethereum transactions to the L1 smart contract
