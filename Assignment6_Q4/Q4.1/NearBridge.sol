...
/* [Infrastructure only] Comment code implemented in NearBridge.sol. Note that they're using fraud proofs and not zk proofs.
*/
...
/// contract NearBridge is an immplementation of near light client in solidity on ethereum chain
...
/// "struct Epoch" creates a new data type Epoch that contains different data types, Representing
/// Near's epoch when validators of the Near's network remain constant.
    struct Epoch {
        bytes32 epochId;
        uint numBPs;
...
/// Deposit Eth tokens and update the balance of depositor
...
/// Withdraw Near tokens and update the balance of reciever
...
/// The user submitting the NEAR header would have to post a bond in Ethereum tokens, and a successful 
/// challenge would burn half of the bond and return the other half to the challenger. The bond 
/// should be large enough to pay for the gas even if the gas price increases exponentially 
/// during the 4 hours
...
/// watchdog service that monitors submitted NEAR headers and challenges any headers with 
/// invalid signatures.
...
/// Initialization for the validators that can be called but overrided by the admin only.
...
/// Initialization for Near's chain instance "blocks" that can be called but overrided by the admin only.
...
/// "struct BridgeState" creates a new data type BridgeState that contains different data types, 
///  Representing the Bridge instance inside the contract.
...
/// Returns the bridge state and view it publicly to bridge contracts.
...
/// A canonical of blocks form of light client instance to monitor the bridge state.
...
/// Set the block producers of Near's validators for the light client instance, that will eventually 
...
/// Immplementation of Near's chain blockhashes to be checked by both block producers & the light client
...
/// Immplementation of Near's blocks merkle roots to be checked by the light client immplementation.
