...
/// manual care must be taken to not invoke a parent initializer twice like EthereumLightClient.
import "@openzeppelin/contracts-upgradeable/proxy/Initializable.sol";
/// Grants `DEFAULT_ADMIN_ROLE`, `PAUSER_ROLE` to the admin account.
import "@openzeppelin/contracts-upgradeable/utils/PausableUpgradeable.sol";
/// Ethereum parser to Parse rlp-encoded structures of data.
import "./EthereumParser.sol";
import "./lib/EthUtils.sol";
/// It verifies if a share (a smaller POW than the real block) was mined according to the ethash algorithm.
import "./ethash/ethash.sol";

/// @title Ethereum light client
/// Immplementation of Ethereum's light client as solidity contract on harmony
/// inherits from Initializable, PausableUpgradeable and Ethash contracts
contract EthereumLightClient is Ethash, Initializable, PausableUpgradeable {
    using SafeMathUpgradeable for uint256;

/// "struct StoredBlockHeader" compose a new data type StoredBlockHeader that contains different data types, 
/// Representing Ethereum's chain instance stored block header.    
...
/// "struct HeaderInfo" compose a new data type HeaderInfo that contains different data types, 
/// Representing Ethereum's chain instance appended header info table, about block difficulty, parent hash of the current block and block number. 
    struct HeaderInfo {
    ...
    /// Set the finality of the EVM instance to 13 blocks (TODO)
   ...
   // Set the first block in the canonical chain of blocks of the EVM instace.
