...
void Equal_Suites_5_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Equal_Suites_5_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_size_of_witness() {return 3968;}
...
ctx->componentMemory[coffset].templateName = "Equal_Suites";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "card1";
mySubcomponents[aux_create+i] = aux_cmp_num;
CommitCard_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 2658 ;
aux_cmp_num += 5;
}
}
...
uint csoffset = mySignalStart+5324;
for (uint i = 0; i < 1; i++) {
...
// need to run sub component
assert(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
CommitCard_2_run(mySubcomponents[cmp_index_ref],ctx);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
...
Equal_Suites_5_create(1,0,ctx,"main",0);
Equal_Suites_5_run(0,ctx);
}
